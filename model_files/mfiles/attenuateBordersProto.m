function result = attenuateBordersProto(data,borderSize)
...
%Rate of attenuation (higher means slower attenuation)
atten_rate = 7;

bs = [1:borderSize];
coeffs = ((bs .* atten_rate) + ((borderSize  - bs) .* (atten_rate - 1))) / ((borderSize * atten_rate) + 1);
